...
async function ensureLeadSubmissionsSchema() {
  if (!process.env.DATABASE_URL) return
  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS "lead_submissions" (
        "id" serial PRIMARY KEY NOT NULL, "idempotencia" varchar NOT NULL,
        "enviado_em" timestamp(3) with time zone NOT NULL,
        "escritorio" "enum_lead_submissions_escritorio" NOT NULL,
        "telefone" varchar NOT NULL, "nome" varchar NOT NULL, "email" varchar,
        "campanha" varchar, "origem" "enum_lead_submissions_origem" NOT NULL,
        "utm_source" varchar, "utm_medium" varchar, "utm_campaign" varchar,
        "utm_content" varchar, "utm_term" varchar, "referrer" varchar,
        "consent_aceito" boolean DEFAULT false NOT NULL, "consent_versao" varchar NOT NULL,
        "consent_em" timestamp(3) with time zone NOT NULL, "consent_ip" varchar,
      ...
      CREATE UNIQUE INDEX IF NOT EXISTS "lead_submissions_idempotencia_idx" ON "lead_submissions" ("idempotencia");
      CREATE INDEX IF NOT EXISTS "lead_submissions_respostas_order_idx" ON "lead_submissions_respostas" ("_order");
      CREATE INDEX IF NOT EXISTS "lead_submissions_respostas_parent_id_idx" ON "lead_submissions_respostas" ("_parent_id");
      CREATE INDEX IF NOT EXISTS "lead_submissions_updated_at_idx" ON "lead_submissions" ("updated_at");
      CREATE INDEX IF NOT EXISTS "lead_submissions_created_at_idx" ON "lead_submissions" ("created_at");
      CREATE INDEX IF NOT EXISTS "payload_locked_documents_rels_lead_submissions_id_idx" ON "payload_locked_documents_rels" ("lead_submissions_id");
    `)
    console.log('[db:startup] Schema lead_submissions pronto.')
  } finally {
    await client.end()
  }
}
